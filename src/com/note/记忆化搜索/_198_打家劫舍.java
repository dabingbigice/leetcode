    ...
    * */
    int dfs(int idx, int[] nums) {
        ...
        /*
         nums = [1, 2, 3]，
        * 计算 dfs(0)
          偷index0: 金额 = 1 + dfs(2)  // ##需计算dfs(2)##
          不偷index0: 金额 = dfs(1)     // 需计算dfs(1)

        计算 dfs(1)
          偷index1: 金额 = 2 + dfs(3) = 2 + 0 = 2
          不偷index1: 金额 = dfs(2)     // ##需再次计算dfs(2)！##
          结果 = max(2, dfs(2))

         dfs(idx + 2, nums)
         dfs(2)被计算了两次：
          第一次由 dfs(0)的"偷"分支触发。
          第二次由 dfs(1)的"不偷"分支触发。
          若数组更长（如 nums = [1, 2, 3, 4]），dfs(2)会被更多分支重复计算（如 dfs(0)、dfs(1)均可能调用它），dfs(3)也会被多次调用
        * */
